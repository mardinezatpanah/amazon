    ...
    <div className="w-full h-20 bg-black sticky top-0 z-50 border-b-[1px] border-b-white">
      <nav className="h-full px-4 max-w-container mx-auto relative">
        <div className="flex items-center justify-between h-full">
          <Link to="/">
            <div>
              <img className="w-20 object-cover" src="/amazon.jfif" />
            </div>
          </Link>
          <div>
            {showMenu && (
                ...
                >
                  <div className="w-full h-full p-6">
                    ...
                    <ul className="text-gray-200 flex flex-col gap-2">
                      {navBarList.map((item) => (
                        <li
                          className="font-normal hover:font-bold items-center text-lg text-rray-200 hover:underline underline-offset-[4px] decoration-[1px] hover:text-white md:border-r-[2px] border-r-gray-300 hoverEffect last:border-r-0"
                          key={item._id}
                        >
                          <NavLink
                            to={item.link}
